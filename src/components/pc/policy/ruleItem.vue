    ...
    <div class="pages_ruleItem">
        <div class="ruleList" v-for="(item,index) in ruleList" :id="'ruleList'+item.id" :key="index">
            ...
            <el-form-item class="ruleName mg_b_0 pointer"
                          :label="($route.name=='classifyForm'||$route.name=='parallelForm'||$route.name=='FlowClassifyForm')?(A_Z[index]+'_'):(index+1+'_')">
                <div class="ruleNameSpan " @click.self="openRuleCon(item,index)">
                    ...
                    <el-input class="w310" v-else-if="$route.name=='classifyForm'||$route.name=='FlowClassifyForm'" v-model="item.ruleName"
        ...
        <!--<div class="noResult mg_b_20">查询无结果，去<span class="f_blue pointer" @click="show.ruleForm=true">新建规则</span>-->
        <!--</div>-->
    </div>
</template>
    ...
    export default {
        ...
        methods: {
            ...
            openRuleCon(item, index) {// 展开规则内容
                if (item.id != ''||item.ruleContent[0].variableId!='') {
