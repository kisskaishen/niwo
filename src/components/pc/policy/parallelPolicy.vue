<template>
    <div class="pages_policyItem" ref="parallelPolicy">
        <div class="classifyList mg_b_10" v-for="(item,index) in classifyForm.policyList" :id="'classifyList'+item.id"
            ...
            <el-form-item class="classifyName mg_b_0" :label="index+1+'_'">
                <div class="classifyNameSpan pointer" style="line-height: 40px;"
                    ...
                    <el-input class="w310" v-else-if="$route.name=='classifyForm'" v-model="item.policyName"
            ...
            <div class="classifyContent clear" :class="{'open':item.policyShow}" :id="'classifyContent'+item.id"
                ...
                <div class="classify_bd mg_t_20">
                    <el-form-item :label="$route.name=='classifyForm'?'分类执行条件：':'策略执行条件：'" prop="logic"
                                  class="classifyLogic">
...
<script>
    ...
    import ruleItem from '../../../components/pc/policy/parallelPolicyItem'
    ...
    export default {
        ...
        created() {
            ...
            this.getVariableList(); // 查询查询变量列表
            this.getOprList_enumList()
            this.variableList = this.variableListUml;           //从父级拿到的变量列表
            this.ruleTmplList = this.ruleTmplListUml;
            // !this.variableListUml.length && this.getVariableList(); // 查询查询变量列表
        },
        methods: {
            stratAdd() {
            ...
            changePolicyName(item, index) {// 要把policyName传给后台
                ...
                this.$refs.ruleItem[0].getRuleModelList()
            },
            /************************ policyItem专用的方法 ************************/
            ...
            validateStrat(stratForm) {
                if (this.hasRepeat) {// 有重复项
                    ...
                    let ruleNum = stratForm.ruleList.filter((rule) => {
                        ...
                        if (this.$route.name == 'classifyForm') {// 分类器的分类的规则名称选填
        ...
        watch: {
            policyNameArr(val) {
                val.map((item, index) => {
                    this.policyListFilter.push({id: '-1', policyName: val[index]})
                })
