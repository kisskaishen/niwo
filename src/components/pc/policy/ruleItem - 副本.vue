<template>
    <div class="pages_ruleItem">
        <div class="ruleList" v-for="(item,index) in ruleList" :id="'ruleList'+item.id" :key="index"
            ...
            <!-- 规则标题 -->
            <el-form-item class="ruleName mg_b_0 pointer" :class="{'bottom':item.ruleConShow}"
                ...
                <div class="ruleNameSpan " @click.self="openRuleCon(item,index)">
                    ...
                    <el-input class="w310" v-else-if="$route.name=='classifyForm'" v-model="item.ruleName" style="margin-left: 12px"
                              placeholder="请输入规则名称（选填）"
                              size="medium" :disabled="($route.query.type=='look')" clearable></el-input>
                    <!--这里是规则名称-->
                    <el-select v-else ref="ruleName" :popper-append-to-body="true" class="w310" style="margin-left: 12px"
                               :class="{'solid_red':vLock&&!item.id}" v-model.number="item.id" filterable
