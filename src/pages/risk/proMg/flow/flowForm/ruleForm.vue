...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            /************************ ruleForm专用的方法 ************************/
            arrowLeft() {// 返回
                if (this.$route.name == 'strat_ruleForm') {// 返回策略
                    this.$router.push({path: '/risk/proMg/businessList/editFlow//stratForm', query: this.$route.query});
                } else if (this.$route.name == 'parallelForm_ruleForm') {// 返回策略
                    this.$router.push({
                        path: '/risk/proMg/businessList/editFlow//parallelForm',
                        query: this.$route.query
                    });
                } else if (this.$route.name == 'stratAdd_ruleForm') {// 返回策略
                    this.$router.push({path: '/risk/stratMg/stratList/stratAdd', query: this.$route.query});
                }
            },
            saveRule() {// 保存规则
                if (this.validateRule(this.ruleForm)) {// 新建规则验证
                    console.log("规则保存》》》", this.$route.name, this.ruleForm.isTemplate);
                    if (this.$route.name == 'ruleTmpAdd' || this.$route.name == 'ruleTmpEdit' || this.$route.name.indexOf('_ruleForm') > -1) {// 新增规则模板调用
            ...
            createRule() {
                console.log("创建规则");
                console.log(this.ruleForm.appId)
                alert(this.ruleForm.appId)
                if (!this.ruleForm.appId) {
                    this.$message.warning('请先选择产品线')
                } else {
                    this.$post('/mcs/rule/create', this.ruleForm)
                        .then(res => {
                            this.ruleForm.id = res.id;
                            console.log("创建规则成功，调用createRuleSuccess渲染", this.ruleForm);
                            this.$emit('createRuleSuccess', this.ruleForm)
