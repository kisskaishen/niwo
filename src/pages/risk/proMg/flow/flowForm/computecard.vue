        ...
        <el-form class="computecard" :model="computecard" status-icon ref="computecard">            
            <el-form-item class="computeItem" label="常用公式：" prop="formula" v-if="$route.query.type=='edit'">
                <el-radio-group v-model="computecard.formula">
                    <el-radio :label="item.value" v-for="(item,index) in formulaList" :key="index" v-if="item.common">{{item.text}}</el-radio>
                </el-radio-group>
            </el-form-item>
            ...
            <el-form-item class="computeItem" label="公式：" prop="formula" v-else>
                <el-radio-group v-model="computecard.formula">
                    <el-radio :label="item.value" v-for="(item,index) in formulaList" :key="index" v-if="item.value==computecard.formula">{{item.text}}</el-radio>
                </el-radio-group>
            </el-form-item>
            <el-form-item class="computeItem" label="计算内容：" prop="value" v-if="computecard.formula">
                ...
                <el-form-item label="" v-else>
                    <div class="value_pos inlineBlock" :class="computecard.computeType==1?'left':'right'">
                        <el-input class="w150" :class="{'solid_red':vLock&&!value.text}" v-model="value.text" placeholder="请输入自然数幂" size="medium" @input="valueInput" @keyup.native="value.text=value.text.replace(/[^\d./-]/g,'')" :disabled="$route.query.type=='look'" clearable></el-input>
                        <img class="icon_search pointer" :src="icon.search" @click="dialog.show=true" alt="搜索" v-if="$route.query.type=='edit'">
                    </div>
                    <span class="comma">，</span>
                    <span class="value_pos w150 el-input el-input--medium text-middle" :class="computecard.computeType==1?'right':'left'">
                        <span class="el-input__inner" :class="{'noBorder':$route.query.type=='look'}">前一个元件输出</span>
                    </span>
                    <img class="icon_refresh pointer" :src="icon.refresh" alt="转换计算方式" @click="changeType" v-if="$route.query.type=='edit'">
                </el-form-item>
            </el-form-item>
            <div class="text-center">
                <el-button class="" type="primary" size="medium" @click="bindComputecard" v-if="$route.query.type!='look'">保存</el-button>
            </div>
        </el-form>
        <!-- 弹窗区域 -->
        <el-dialog title="选择变量" :visible.sync="dialog.show" :modal-append-to-body="false" width="400px" center>         
            <el-select :popper-append-to-body="true" class="w100" v-model="value.select" ref="changeStratVar" filterable placeholder="请选择已有变量" size="medium" no-match-text="查询无结果" no-data-text="暂无数据" popper-class="stratList">
                <el-option v-for="(item,index) in variableList" :key="index" :label="item.variableName" :value="item.variableId"><div class="stratOption overhide" :title="item.variableName">{{item.variableName}}</div></el-option>
            </el-select>
    ...
    methods:{
        ...
        bindComputecard () {   
            if(!this.computecard.formula){
                ...
                // axios({
                //     url:'/mcs/node/computecard/bind',
                //     data,
                //     debug:true,
                //     loadingDom:'.computecard',
                //     // ret:0,
                // },(res)=>{
                //     data.nodeName = '计算卡';
                //     data.nodeId = this.$route.query.nodeId;
        ...
        queryComputecard () {    
            ...
            // axios({
            //     url:'/mcs/node/computecard/query',
            //     data:{
            //         nodeId:this.$route.query.nodeId.slice(4),// 产品线ID   Number  Y
            //     },
            //     loadingDom:'.computecard',
            ...
            // },(res)=>{
            //     if(res.data){
            //         Object.assign(this.computecard,res.data);
            //         if(res.data.valueType==1){
            //             this.value.text = res.data.value;
            //         }else if(res.data.valueType==2){
            //             this.value.text = this.variableList.filter(item=>item.variableId==res.data.value)[0].variableName
            //         }
            //     }
            //     {// 撤销恢复：记录绑定的初始版本
            //         let data = JSON.parse(JSON.stringify(this.computecard))
