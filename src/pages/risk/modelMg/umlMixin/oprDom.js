export const oprDom = {
    methods:{
        createNode(item) {
            ...
            let kingDiv = '';
            if(node.dataset.nodetype==4||node.dataset.nodetype==5){
                if (item.hasChallenger=='0'||item.hasChallenger==null) {
                    kingDiv = `<div class="kingDiv">
								<i class="iconAdd" data-modal='/risk/modelMg/modelUml/kingForm' onclick="_self.switchForm(event)"></i>
...
`
            ...
            switch(item.nodeType*1){// 1：开始，2：结束，3：分类器，4：策略，5：评分卡，6：计算卡，7：AB测试，8：并行
                case 1:
                ...
                case 3:
                    ...
                    node.dataset.modal = '/risk/modelMg/modelUml/classifyForm';
                    break;
                case 4:
                    node.className = 'uml uml_strat';
                    node.dataset.modal = '/risk/modelMg/modelUml/stratForm';
                    break;
                case 5:
                    node.className = 'uml uml_scoreCard';
                    node.dataset.modal = '/risk/modelMg/modelUml/scoreCardForm';
                    node.dataset.needmoldtype = '4';
                    break;
                case 6:
                    node.className = 'uml uml_calculator';
                    node.dataset.modal = '/risk/modelMg/modelUml/computecard';
                    break;
                // case 7:
                //     node.className = 'uml uml_test';
                //     node.dataset.modal = '/risk/modelMg/modelUml/testAB';
                //     break;
                case 7:
                    node.className = 'uml uml_parallel';
                    node.dataset.modal = '/risk/modelMg/modelUml/parallelForm';
                    break;
            }
...
export const umlDomFn = {
    methods:{
        ...
        keydown(){
            document.onkeyup = (e)=> {
                console.log('_______________：keydown')
                var currKey = 0;
        ...
        lineFn() {// 连线相关方法
            jsPlumb.bind("click",(info,event)=>{
                ...
                if(info.source.dataset.nodetype==3){// 点击分类器的线
                    ...
                    info.canvas.dataset.modal = '/risk/modelMg/modelUml/selectClassify';
