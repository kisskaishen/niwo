...
export const ajax = {
        ...
        return {
            ...
            ajax: {
                createNode: (nodeData) => {// 应用场景：创建节点——撤销——恢复
                    this.$post('/mcs/node/create', {
                        ...
                        .then(res => {
                            ...
                            // this.allContentList = this.content.nodeList.push(res)
                        })
                },
                ...
                bindNode: (nodeData) => {
                    ...
                    data.formtype = nodeData.formtype
                    if (data.formType2 == 'scoreKing' && !data.bindMoldList) {
                        data.bindMoldList = data.data ? data.data : data.bindMoldList

                    } else if (!data.policyList) {
                        data.policyList = data.data ? data.data : data.policyList

                    }
                    if(!!data.bindMoldList) {
                        data.policyList = data.bindMoldList
                    } else if (!!data.policyList) {
                        data.bindMoldList = data.policyList
                    }
                    // data.bindMoldList = data.bindMoldList = data.data?data.data:data.bindMoldList
                    console.log(data)

                    if (data.policyList&&!!data.decisions) {// 格式化数据
                        data.policyList.map(pItem => {
                            pItem.id = ''
                            if (!!pItem.ruleList) {
                ...
                deleteNode: (nodeData) => {// 3.2.38模型配置—删除节点
                    ...
                    this.$post('/mcs/node/delete', {
                        ...
                        .then(res => {
                            ...
                            this.checkOnlyNode();// 检查唯一节点是否存在
                            // 下面就是要开始只删除节点，或者重新请求全部重绘，请求content方法
                            setTimeout(() => {
                                jsPlumb.remove(nodeData.id);
                            }, 100)
                        })
                ...
                createLine: (connection) => {// 3.2.40模型配置—连线
                    if (this.nodeDirect.indexOf(connection.endpoints[1].anchor.type) > -1) {
                        ...
                        this.$post('/mcs/node/line/create', {
                            moldId: this.$route.query.modelId,// 模型Id
                    ...
                    jsPlumb.getConnections().map((info, index) => {
                        if (item.id == info.id) {
                            this.$post('/mcs/node/line/delete', {
                                ...
                                .then(res => {
                                    // console.log('删除',info)
                                    this.pushLine(item, 'delete');// 写入记录
    ...
    methods: {
        ...
        getBindData(msg) {
            ...
            //         // 撤销恢复
            //         nodetype: 4,
            //         nodeType: 4,// 首次渲染的createNode(nodeType)和恢复的createNode(nodetype)造成大小写兼容问题
            //         oprtype: 'bind',
            //         formData: msg,
            //         clear: !!msg.clear,// true:清空(res.data无数据&&要还原为初始数据),false:不清空
            //     }
